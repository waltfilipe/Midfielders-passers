"""Pass Scout — World Cup position ratings and European midfielder analysis."""
...
def _xp_compare_metric_label_html(label: str, key: str) -> str:
    tip = (
        xstats.XP_COMPARE_COLUMN_TOOLTIPS.get(key)
        or xstats.XP_COMPARE_HIGHLIGHT_TOOLTIPS.get(key)
...
_COMPARE_PROFILE_KEYS: tuple[str, ...] = (
    "dominant_foot",
    "age",
    "height",
    "nationality",
...
_COMPARE_PROFILE_SHORT_LABELS: dict[str, str] = {
    "dominant_foot": "Foot",
    "age": "Age",
    "height": "Height",
    "nationality": "Nation",
...
_COMPARE_PROFILE_ICONS: dict[str, str] = {
    "dominant_foot": "fa-shoe-prints",
    "age": "fa-cake-candles",
...
def _player_origin_heatmap_b64(
    player_id: str,
    ...
    return _fig_to_b64(fig_origin)


def _compare_profile_plain_value(player: dict, key: str, *, fmt_pct_fn) -> str:
    if key == "minutes":
        minutes = player.get("minutes")
        minutes_pct = player.get("minutes_pct")
        if minutes is None:
            return "—"
        main = str(int(round(float(minutes))))
        if minutes_pct is not None:
            return f"{main} ({fmt_pct_fn(minutes_pct)})"
        return main
...
def _compare_column_head_html(player: dict, *, variant: str) -> str:
    ...
    return (
        f'<div class="pa-compare-col-head pa-compare-col-head-{html.escape(variant)}">'
        f'<div class="pa-compare-photo-wrap">{_player_photo_html(player)}</div>'
        '<div class="pa-compare-col-head-text">'
        f'<div class="pa-compare-player-name">{html.escape(str(player.get("player_name", "—")))}</div>'
        '<div class="pa-compare-player-sub">'
...
def _compare_column_heatmap_html(heatmap_b64: str | None) -> str:
    if heatmap_b64:
        return (
            '<div class="pa-compare-col-heatmap">'
            f'<img class="pa-compare-heatmap" src="data:image/png;base64,{heatmap_b64}" '
            'alt="Pass origin heatmap" />'
            "</div>"
        )
    return '<p class="pa-compare-heatmap-empty">Heatmap unavailable</p>'


def _compare_column_metric_html(
    label: str,
    source: dict,
    key: str,
    *,
    ...
    bar_block = (
        f'<span class="pa-xp-compare-mini-bar-wrap">{bar}</span>' if bar else ""
    )
    label_html = _xp_compare_metric_label_html(label, key) if highlight else html.escape(label)
    item_cls = "pa-compare-col-metric pa-compare-col-metric-highlight" if highlight else "pa-compare-col-metric"
    return (
        f'<div class="{item_cls}">'
        f'<span class="pa-compare-col-metric-label">{label_html}</span>'
        '<span class="pa-compare-col-metric-body">'
        f'<span class="pa-compare-col-metric-value">{value}{arrow}</span>'
        f"{bar_block}"
        "</span>"
        "</div>"
    )
...
def _compare_column_score_accordion_html(
    source: dict,
    *,
    variant: str,
    display_key: str,
    ...
    letter = source.get(letter_key)
    display_score = source.get(display_key)
    try:
        score_val = float(display_score) if display_score is not None else None
    except (TypeError, ValueError):
        score_val = None
    pill = _pa_letter_grade_pill_html(
        score_val,
        str(letter) if letter else None,
    )
    stat_lines = "".join(
        '<div class="pa-compare-score-stat">'
        f'<span class="pa-compare-score-stat-label">{html.escape(XP_PA_REGULAR_COMPONENT_LABELS.get(key, key))}</span>'
        f'<span class="pa-compare-score-stat-val">{html.escape(_compare_component_stat_value(source, key))}</span>'
        "</div>"
        for key in component_keys
    )
    return (
        f'<details class="pa-compare-score-accordion" name="pa-compare-scores-{html.escape(variant)}">'
        "<summary>"
        '<i class="fa-solid fa-chevron-right pa-compare-score-arrow" aria-hidden="true"></i>'
        f'<span class="pa-compare-score-title">{html.escape(title)}</span>'
        f'<span class="pa-compare-score-pill-wrap">{pill}</span>'
        "</summary>"
        f'<div class="pa-compare-score-body">{stat_lines}</div>'
        "</details>"
    )


def _compare_column_scores_html(source: dict, *, variant: str) -> str:
    blocks = "".join(
        _compare_column_score_accordion_html(
            source,
            variant=variant,
        ...
        for display_key, _index_key, letter_key, title, component_keys in XP_PA_REGULAR_SCORE_SPECS
    )
    return (
        '<div class="pa-compare-col-section">Pass scores</div>'
        f'<div class="pa-compare-col-scores">{blocks}</div>'
    )


def _compare_column_xp_metrics_html(
    source: dict,
    *,
    other_source: dict | None = None,
) -> str:
    rows = "".join(
        _compare_column_metric_html(
            label,
            source,
            key,
            other_source=other_source,
            highlight=key in xstats.XP_PROFILE_BAR_KEYS,
        )
        for key, label in xstats.XP_COMPARE_COLUMN_KEYS
    )
    return (
        '<div class="pa-compare-col-section">xP metrics</div>'
        f'<div class="pa-compare-col-metrics">{rows}</div>'
    )
...
def _build_pa_compare_layout_html(
    ...
    *,
    primary_heatmap_b64: str | None,
    compare_heatmap_b64: str | None,
    fmt_pct_fn,
) -> str:
    return (
        '<div class="pa-compare-layout">'
        '<div class="pa-compare-grid">'
        f"{_compare_column_html(primary_player, primary_source, heatmap_b64=primary_heatmap_b64, variant='primary', fmt_pct_fn=fmt_pct_fn, other_source=secondary_source)}"
        f"{_compare_column_html(compare_player, secondary_source, heatmap_b64=compare_heatmap_b64, variant='secondary', fmt_pct_fn=fmt_pct_fn, other_source=primary_source)}"
        "</div>"
        "</div>"
    )
...
def _render_xp_comparison_panel(
    ...
    *,
    ...
    compare_player = pp.enrich_player_general_profile(compare_player)
    ...
    primary_heatmap_b64 = _player_origin_heatmap_b64(
        primary_id,
        passes_by_player,
        str(primary.get("player_name", "")),
    )
    compare_heatmap_b64 = _player_origin_heatmap_b64(
        compare_id,
        passes_by_player,
        str(compare_player.get("player_name", compare_xp.get("player_name", ""))),
    )
    st.html(
        _build_pa_compare_layout_html(
            primary,
            compare_player,
            primary_source,
            secondary_source,
            primary_heatmap_b64=primary_heatmap_b64,
            compare_heatmap_b64=compare_heatmap_b64,
...
st.markdown(
    ...
    .pa-compare-player-card-primary {
        box-shadow: inset 0 1px 0 rgba(167, 139, 250, 0.14);
    }
    .pa-compare-player-card-secondary {
    ...
    .pa-pass-mix-axis-compact {
        font-size: 0.56rem;
        margin-top: -0.05rem;
    }
    .pa-compare-col-metrics {
        display: flex;
        flex-direction: column;
        gap: 0;
    ...
    .pa-compare-score-accordion summary {
        display: flex;
        align-items: center;
        gap: 0.55rem;
    ...
    .pa-compare-score-accordion summary::-webkit-details-marker { display: none; }
    .pa-compare-score-arrow {
        color: #64748b;
        font-size: 0.62rem;
        transition: transform 0.14s ease;
        flex-shrink: 0;
    }
    .pa-compare-score-accordion[open] .pa-compare-score-arrow {
        transform: rotate(90deg);
    }
    .pa-compare-score-title {
        flex: 1;
        color: #e2e8f0;
        font-size: 0.8rem;
        font-weight: 700;
    ...
    .pa-compare-score-pill-wrap {
        flex-shrink: 0;
    }
    .pa-compare-score-pill-wrap .pa-letter-grade-pill {
        min-width: 2.1rem;
        padding: 3px 9px;
        font-size: 0.74rem;
    }
    .pa-compare-score-body {
        padding: 0.2rem 0.7rem 0.55rem;
        border-top: 1px solid rgba(59, 130, 246, 0.12);
    }
    .pa-compare-score-stat {
        display: flex;
        justify-content: space-between;
    ...
    .pa-compare-score-stat:last-child { border-bottom: none; }
    .pa-compare-score-stat-label {
        color: #94a3b8;
        font-size: 0.72rem;
        font-weight: 600;
        line-height: 1.25;
    }
    .pa-compare-score-stat-val {
        color: #f8fafc;
        font-size: 0.78rem;
        font-weight: 700;
        text-align: right;
    ...
    .pa-compare-col-metric:last-child { border-bottom: none; }
    .pa-compare-col-metric-highlight .pa-compare-col-metric-label {
        color: #dbeafe;
        font-weight: 800;
    }
    .pa-compare-col-metric-highlight .pa-compare-col-metric-value {
        font-size: 1.02rem;
        color: #f8fafc;
    }
    .pa-compare-col-metric-label {
        color: #94a3b8;
        font-size: 0.72rem;
        font-weight: 700;
        letter-spacing: 0.02em;
    }
    .pa-compare-col-metric-body {
        display: flex;
        flex-direction: column;
        gap: 0.28rem;
        min-width: 0;
    }
    .pa-compare-col-metric-value {
        color: #e2e8f0;
        font-size: 0.92rem;
        font-weight: 700;
        display: inline-flex;
    ...
    .pa-compare-col-metric .pa-xp-compare-mini-bar-wrap {
        display: block;
        width: 100%;
        max-width: 100%;
    }
    .pa-compare-photo-wrap .pa-identity-photo,
    .pa-compare-photo-wrap .pa-identity-photo-placeholder {
        width: 3.6rem;
        height: 3.6rem;
        border-radius: 12px;
        font-size: 0.95rem;
    }
    .pa-compare-player-name {
        color: #f8fafc;
        font-size: 1rem;
        font-weight: 800;
        line-height: 1.2;
    }
    .pa-compare-player-sub {
        margin-top: 0.18rem;
        color: #94a3b8;
        font-size: 0.74rem;
        font-weight: 600;
    ...
    .pa-pillars-card .pa-xp-index-row {
        padding: 0.34rem 0.48rem;
    }
    .pa-pillars-card .pa-xp-index-list {
        gap: 0.22rem;
    }
    .pa-xp-index-title {
    ...
    .pa-xp-index-row-elite {
        border-color: rgba(56, 189, 248, 0.45);
        background: linear-gradient(90deg, rgba(56, 189, 248, 0.12) 0%, rgba(15, 23, 42, 0.45) 100%);
    }
    .pa-xp-index-row-elite .pa-xp-index-row-val {
        color: #38bdf8;
        font-weight: 800;
    }
    .pa-xp-index-row-elite .pa-xp-index-row-icon {
        color: #38bdf8;
    ...
    .pa-xp-index-tip-chart {
        display: block;
        margin-top: 0.42rem;
        padding-top: 0.38rem;
        border-top: 1px solid rgba(51, 65, 85, 0.55);
    }
    .pa-round-chart-wrap-compact { width: 100%; }
    .pa-round-chart-compact {
        display: block;
        width: 100%;
        height: 52px;
        overflow: visible;
    }
    .pa-round-chart-wrap-compact .pa-round-chart-legend {
        margin-top: 0.2rem;
    }
    .pa-xp-index-details {
        border-radius: 9px;
    ...
    .pa-pass-mix-axis-short { color: #7dd3fc; }
    .pa-pass-mix-axis-long { color: #fcd34d; }
    .pa-pass-mix-legend {
        display: flex;
        justify-content: space-between;
        gap: 0.5rem;
...
def _european_midfielder_bundle_or_prompt(*, button_key: str) -> tuple | None:
    if PLAYER_ANALYSIS_BUNDLE_KEY not in st.session_state:
        ...
        if st.button("Load European midfielders", key=button_key, type="primary"):
            with st.spinner("Loading European midfielders…"):
...
def _xp_index_tip_lines_html(
    xp_profile: dict,
    keys: tuple[str, ...],
    *,
    show_rank: bool = True,
) -> str:
    """Metric lines (label, value, optional rank) for a rich index tooltip."""
    lines: list[str] = []
    for key in keys:
...
_ROUND_CHART_H = 88.0
_ROUND_CHART_PAD_X = 8.0
_ROUND_CHART_PAD_Y = 10.0
...
def _round_production_chart_html(
    ...
    *,
    ...
    chart_h = 52.0 if compact else _ROUND_CHART_H
    pad_x = 6.0 if compact else _ROUND_CHART_PAD_X
    pad_y = 7.0 if compact else _ROUND_CHART_PAD_Y
    inner_w = _ROUND_CHART_W - 2 * pad_x
    inner_h = chart_h - 2 * pad_y

    def pos_x(index: int) -> float:
        return pad_x + inner_w * index / (count - 1)

    def pos_y(value: float) -> float:
        return pad_y + inner_h * (1.0 - (value - lo) / span)

    points = " ".join(f"{pos_x(i):.1f},{pos_y(v):.1f}" for i, v in enumerate(values))
    mean_y = pos_y(mean)
    dots = "".join(
        f'<circle class="pa-round-chart-dot" '
        f'cx="{pos_x(i):.1f}" cy="{pos_y(v):.1f}" r="{1.4 if compact else 1.6}">'
        f"<title>{html.escape(_round_point_title(series[i]))}</title>"
        "</circle>"
        for i, v in enumerate(values)
    )
    wrap_cls = "pa-round-chart-wrap pa-round-chart-wrap-compact" if compact else "pa-round-chart-wrap"
    legend_html = (
        f'<span class="pa-round-chart-legend-item pa-round-chart-legend-mean">μ {mean:.2f} xP</span>'
        if compact
        else (
            f'<span class="pa-round-chart-legend-item pa-round-chart-legend-mean">μ {mean:.2f} xP</span>'
            f'<span class="pa-round-chart-legend-item">{count} games</span>'
        )
    )
    return (
        f'<div class="{wrap_cls}">'
        f'<svg class="pa-round-chart{" pa-round-chart-compact" if compact else ""}" '
        f'viewBox="0 0 {_ROUND_CHART_W:.0f} {chart_h:.0f}" '
        'role="img" preserveAspectRatio="none" '
        f'aria-label="xP per game across {count} matches">'
        f'<line class="pa-round-chart-mean" x1="{pad_x:.1f}" y1="{mean_y:.1f}" '
        f'x2="{_ROUND_CHART_W - pad_x:.1f}" y2="{mean_y:.1f}"></line>'
        f'<polyline class="pa-round-chart-line" points="{points}"></polyline>'
        f"{dots}"
        "</svg>"
        f'<div class="pa-round-chart-legend">{legend_html}</div>'
        "</div>"
    )
...
def _xp_index_boxes_html(
    ...
    if not xp_profile or not xp_profile.get("xp_profile_bars_eligible", True):
        return ""
    rows: list[str] = [
        _xp_consistency_row_html(xp_profile, xp_passes_df),
        _xp_impact_pass_row_html(xp_profile),
    ]
    rows = [row for row in rows if row]
    if not rows:
...
def _pa_pass_length_card_html(xp_profile: dict | None) -> str:
    ...
    return (
        ...
        f"<strong>{short_share:.1f}%</strong> short</span>"
    ...
    *,
    ...
    with st.container(key="pa_filter_card"):
        st.markdown(
            ...
            '<span class="pa-filter-sub">Refine o grupo de meio-campistas '
        ...
        position_codes, position_groups = _all_position_filters()
        options = _player_analysis_options(
            pool,
            progression_by_id,
            position_codes=position_codes,
...
def _render_xp_maps_analysis_content() -> None:
    """Aggregated xP maps for European midfielders."""
    ...
    st.markdown(
        ...
        "Visão agregada — Meio-campistas</div>",
        unsafe_allow_html=True,
    )

    map_title_common = f"Passes mais comuns · destino ({player_count} meio-campistas)"
    map_title_rare = f"Passes mais raros · xP médio no destino ({player_count} meio-campistas)"
